...
`discord_instants_player.iss` is an Inno Setup script used to build a Windows installer that bundles just the built binary — no external runtime dependency to bundle anymore, now that `pkg/opusaudio` decodes and resamples in pure Go (see "Architecture" above). `.github/workflows/ci.yaml`'s `build-windows` job compiles `discord_instants_player.exe` on a `windows-latest` runner on every push and pull request; since the whole module has been cgo-free for a while, this could become a cross-compiled step in the main job instead, but that's a CI-architecture change, not something this doc should assume happened.
