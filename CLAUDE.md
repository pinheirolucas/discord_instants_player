...
- Errors surfaced from `pkg/instant`/`pkg/fsutil` (`ErrInvalidLink`, `fsutil.ErrNotFound`, `fsutil.ErrUnsuportedAudioFormat`) are mapped to specific HTTP status codes/labels in `pkg/server/server.go` — follow that pattern when adding new error cases rather than falling through to the generic 500. `writeErrorMessage` writes the status it is given, and the UI's HTTP client (in the sibling `discord_instants_player_ui` repo) is expected to branch on it; `label` remains the stable machine-readable identifier within a given status. See "Internationalization" below for where the `message` text that goes with each `label` actually lives.
...
`discord_instants_player.iss` is an Inno Setup script used to build a Windows installer that bundles the built binary with `ffmpeg`. `.github/workflows/ci.yaml`'s `build-windows` job compiles `discord_instants_player.exe` on a `windows-latest` runner on every push and pull request; now that `pkg/opusaudio` no longer depends on cgo (see "Architecture" above), this could become a cross-compiled step in the main job instead, but that's a CI-architecture change, not something this doc should assume happened. The separate `ffmpeg` job that downloads the ffmpeg bundle is still `workflow_dispatch`-only and has never run automatically.
